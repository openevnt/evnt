...
cli.command("validate [file]", "Validate event file(s) against the schema")
...
cli.command("show [file]", "Pretty-print an event")
...
cli.command("convert [file]", "Convert an event to another format")
...
cli.command("build [dir]", "Build a static site from event files")
...
cli.command("check [file]", "Lint event file(s)")
	.action(async (file?: string) => {
		if (file && !file.endsWith(".json")) {
...
cli.command("new", "Interactively create a new event file")
