...
export const PhysicalVenueSchema = z
	.object({
		...
		address: z
			.object({
				countryCode: z
					.string()
					.optional()
					.meta({ description: "The ISO 3166-1 alpha-2 country code" }),
				postalCode: z
					.string()
					.optional()
					.meta({ description: "The postal code of the address, if any" }),
				addr: z
					.string()
					.optional()
					.meta({ description: "Full address excluding country and postal code" }),
			})
			.optional(),
		maps: z
			...
			.meta({
				description:
					"Map service identifiers keyed by NSID (e.g. `org.openstreetmap.node`, `com.google.places`). Values are the identifier(s) assigned by that service for this location.",
			}),
	})
	.meta({
		id: "PhysicalVenue",
